...
from ...data_handlers import EventLogDataHandler
...
def fn_combined_results_global(param):
    ...
    return EVENT_LOG_ROOT / 'common' / f'{param}_results.csv'
...
def fn_combined_results_time_mean(param):
    ...
    """
    return EVENT_LOG_ROOT / 'common' / f'{param}_time_mean.csv'
...
def fn_combined_results_time_ci(param):
    ...
    """
    return EVENT_LOG_ROOT / 'common' / f'{param}_time_ci.csv'


def all_completed_cdh(param: [str, None]) -> Iterable[EventLogDataHandler]:
    """
    ...
    Returns
    -------
    cdh_list: Iterable[EventLogDataHandler]
        Collection of ConsumerDataHandlers that are of the correct experiment type, and are finished.

    """
    if not (EVENT_LOG_ROOT / 'individual').exists():
        return []
    if param == 'rho':
        ret = map(EventLogDataHandler,
                  filter(lambda x: '_rho' in x, file_functions.list_dirs(EVENT_LOG_ROOT / 'individual', False)))
    elif param == 'k':
        ret = map(EventLogDataHandler,
                  filter(lambda x: '_k' in x, file_functions.list_dirs(EVENT_LOG_ROOT / 'individual', False)))
    elif param is None:
        ret = map(EventLogDataHandler, file_functions.list_dirs(EVENT_LOG_ROOT / 'individual', False))
    else:
